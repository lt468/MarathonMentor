...
| 30/10/2023    | Working on the homepage and implementing showing the runs, need to: 1) Verify what the difference is for Distance and interval runs to show it on the screen 2) Implement the "Upcoming runs..." to show the next 3 runs on the homepage 3) Use JS on the Scheduled Runs (ex. calendar) to display all the schedueled runs by month 4) Find a way to implement the "completed runs" and then update the home page, scheduled runs, and completed runs 5) Strava integration and auto complete the runs |To note: What happens to the home page when I finish a plan, what happens to the upcomming runs after I finish a plan, what happens to the pages after I finish a plan| Might be worth adding some colouring onto the runs so visually you can see what zones you will be working in|
