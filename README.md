...
| 04/11/2023    | I have managed to use the strava api to get the activities from a user | Need to get the activities for the user, get the runs, verify the distance/duration/(other data for interval runs) and then use that to auto detect if a run is completed and add a completed runs section||
