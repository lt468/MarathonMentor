...
| 02/11/2023    | Today's work: 4) Find a way to implement the "completed runs" and then update the home page, scheduled runs, and completed runs 5) Strava integration and auto complete the runs |||
| 04/11/2023    | I have managed to use the strava api to get the activities from a user | Need to get the activities for the user, get the runs, verify the distance/duration/(other data for interval runs) and then use that to auto detect if a run is completed and add a completed runs section. Also integrate the strava account to the runneruser account and link them ||
| 05/11/2023    | Have started to implement a crispy form completed run | Will do the Strava applications after I can manually do this, but for now I need to make sure the user can only get the right run to mark as complete ||
